...
# Import load_dotenv for local .env file usage
...
# IMPORTANT: Keys are now loaded from environment variables (from .env locally, or Streamlit secrets on cloud)
...
async def call_gemini_api(session, prompt, response_schema=None):
    ...
    if not GEMINI_API_KEY:
        st.error("Gemini API Key is missing. Please set GEMINI_API_KEY in your .env file (local) or Streamlit secrets (cloud).")
        return None
...
async def perform_google_search(session, query, num_results=3): # Limit to top 3 articles
    ...
    if not GOOGLE_API_KEY or not CSE_ID:
        st.error("Google API Key or CSE ID is missing. Please set GOOGLE_API_KEY and CSE_ID in your .env file (local) or Streamlit secrets (cloud).")
        return {"web": [], "youtube": []}
...
async def search_youtube_videos(session, query, num_results=3): # Limit to top 3 videos
    ...
    if not GOOGLE_API_KEY:
        st.error("Google API Key is missing for YouTube search. Please set GOOGLE_API_KEY in your .env file (local) or Streamlit secrets (cloud).")
        return []
